...
### HCC and account-specific identity review - 24 July 2026

- A fresh production `Clients / Full audit` started with 60 blocked source rows across 57 unique review decisions and 0 errors.
- Fourteen reviewed HCC service identities were mapped to official Hampshire County Council account `airtable_client_ham007`, Sage `HAM007`: `Children services (willow)`, `Family Practitioner - Hampshire County Council Refugee Team`, `Fareham & Gosport Family Help 1`, `Hampshire Children's Services- Hampshire County Council`, `Hampshire County Council - disabled children's tea`, `Hampshire CSD`, `Hampshire EMTAS`, `Hampshire local authority`, `Hart & Rushmoor Family Help 1 Team`, `HCC Children Services Refugee Team`, `HCC Children Services Refugee Team 2`, `HCC Children's Services - Children's Adoption team`, `HCC Children's Services SEN`, and `HCC, Children's Services, Havant Family Help 4`.
- Airtable department evidence links HCC EMTAS and HCC Children Services directly to official `Clients` record `recbDv7W6MtW9CwKI`. The wider batch was limited to services whose HCC, family-help, refugee-team or children's-services identity was explicit; generic `HCC` prefixes were not accepted on their own.
- `UHS NHS` and `UHS Southampton` were mapped to official Southampton University Hospitals NHS Trust account `airtable_client_sou004`, Sage `SOU004`.
- Orphan department `AIS` was mapped to official Auditory Implant Service account `airtable_client_ais001`, Sage `AIS001`.
- Orphan department `Independant Appeals Service` was mapped to the account-specific HCC Independent Appeals account `airtable_client_ham015`, Sage `HAM015`, rather than general HCC.
- Orphan department `IDVA` was mapped to official Southampton IDVA Service account `airtable_client_sou015`, Sage `SOU015`.
- `Hampshire Hampshire` was explicitly excluded from HCC. Its `hhft.nhs.uk` evidence identifies Hampshire Hospitals NHS Foundation Trust, so the similar display name is a false positive.
- `Footner Ewing` remained unresolved. Airtable has official account `FOO001`, but that target was not present as an existing or approved-pending canonical target in the reviewed platform run; no substitute client was selected.
- The authoritative after-run reported 41 blocked source rows across 38 unique review decisions and 0 errors. The exact reduction of 19 rows and 19 decisions matches the reviewed mappings. Departments increased from 44 to 52 and agents from 1,165 to 1,181 as the resolver could place previously blocked identities.
- Browser validation exposed a fourth confirmation path that still saved an individual `Map existing` decision directly. The test mapping for `Basingstoke & Deane Borough Council` was immediately revoked and a fresh Full Audit returned exactly to 41 blocked rows, 38 decisions and 0 errors; no active test rule remains.
- The Airtable Sync Center now uses the platform confirmation modal for every identity write: individual mapping, recommended batch, manual batch, new-organisation approval and revocation. Individual and batch confirmations identify the selected destination and Sage/client reference before any write.
...
- [x] Apply the reviewed police hierarchy. The canonical account was renamed to `Hampshire and Isle of Wight Constabulary`; `Witness Care Unit` and shared mailbox `wcu.crown@hampshire.police.uk` were created; all three legacy jobs were scoped to the department without inventing a requester. Manifest: `qKkfsZeyafP6Tb9kWJO6`.
- [x] Check Finance Hierarchy before any repair. The reviewed batch reported `0` finance-linked jobs and the client profile reported `0` invoices, so no finance reconciliation was run. The two invoiced workflow statuses remain a separate finance-integrity follow-up and are not evidence of invoice linkage.
