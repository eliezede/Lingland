...
const normalizeServiceCategory = (value) => (String(value || '').trim().toUpperCase() === 'TRANSLATION' ? 'TRANSLATION' : 'INTERPRETATION');
...
exports.generateInterpreterInvoices = functions.https.onCall(async (data, context) => {
    ...
    const candidates = unclassifiedCandidates.filter(item => {
        ...
        if (requestedServiceCategory && category !== requestedServiceCategory)
            return false;
        if (settlementCycleId && String(value.settlementCycleId || '') !== settlementCycleId)
            return false;
        return true;
