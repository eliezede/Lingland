...
exports.opaqueEntityId = exports.suggestionFingerprint = exports.executionPolicyDecision = exports.isExecutionMode = exports.validateAIControlPatch = exports.mergeAIControlConfig = exports.normalizeConfidence = exports.isKnownAIAction = exports.AI_ACTION_REGISTRY = void 0;
...
const scheduleInterval = (value, fallback) => {
    const allowed = [30, 60, 120, 180, 240, 360, 720, 1440];
    const parsed = Number(value);
    return allowed.includes(parsed) ? parsed : fallback;
};
const scheduledScopes = (value) => {
    if (!Array.isArray(value))
        return types_1.DEFAULT_AI_CONTROL_CONFIG.scheduledScopes;
    const scopes = value
        .map(item => String(item).toUpperCase())
        .filter((item) => types_1.AI_REVIEW_SCOPES.includes(item));
    return Array.from(new Set(scopes)).slice(0, types_1.AI_REVIEW_SCOPES.length);
};
const mergeAIControlConfig = (stored) => {
    ...
    return {
        ...
        maxActionsPerRun: boundedInteger(stored?.maxActionsPerRun, types_1.DEFAULT_AI_CONTROL_CONFIG.maxActionsPerRun, 1, 20),
        dailyActionLimit: boundedInteger(stored?.dailyActionLimit, types_1.DEFAULT_AI_CONTROL_CONFIG.dailyActionLimit, 1, 200),
        ...
        scheduleIntervalMinutes: scheduleInterval(stored?.scheduleIntervalMinutes, types_1.DEFAULT_AI_CONTROL_CONFIG.scheduleIntervalMinutes),
...
const validateAIControlPatch = (input) => {
    ...
    if (source.executionEnabled !== undefined)
        patch.executionEnabled = Boolean(source.executionEnabled);
    if (source.externalCommunicationEnabled !== undefined)
        patch.externalCommunicationEnabled = Boolean(source.externalCommunicationEnabled);
    if (source.simulationOnly !== undefined)
        patch.simulationOnly = Boolean(source.simulationOnly);
    if (source.autoExecuteLowRisk !== undefined)
        patch.autoExecuteLowRisk = booleanValue(source.autoExecuteLowRisk, false);
    if (source.autoExecuteMediumRisk !== undefined)
        patch.autoExecuteMediumRisk = booleanValue(source.autoExecuteMediumRisk, false);
    if (source.autoExecuteHighRisk !== undefined)
        patch.autoExecuteHighRisk = booleanValue(source.autoExecuteHighRisk, false);
    if (source.requireApprovalForMediumRisk !== undefined)
        patch.requireApprovalForMediumRisk = booleanValue(source.requireApprovalForMediumRisk, true);
    if (source.requireApprovalForHighRisk !== undefined)
        patch.requireApprovalForHighRisk = booleanValue(source.requireApprovalForHighRisk, true);
    if (source.scheduledReviewsEnabled !== undefined)
        patch.scheduledReviewsEnabled = Boolean(source.scheduledReviewsEnabled);
    if (source.scheduledScopes !== undefined)
        patch.scheduledScopes = scheduledScopes(source.scheduledScopes);
    if (source.scheduleIntervalMinutes !== undefined)
        patch.scheduleIntervalMinutes = scheduleInterval(source.scheduleIntervalMinutes, types_1.DEFAULT_AI_CONTROL_CONFIG.scheduleIntervalMinutes);
    ...
    if (source.maxActionsPerRun !== undefined) {
        patch.maxActionsPerRun = boundedInteger(source.maxActionsPerRun, types_1.DEFAULT_AI_CONTROL_CONFIG.maxActionsPerRun, 1, 20);
    }
    if (source.dailyActionLimit !== undefined) {
        patch.dailyActionLimit = boundedInteger(source.dailyActionLimit, types_1.DEFAULT_AI_CONTROL_CONFIG.dailyActionLimit, 1, 200);
    }
    return patch;
};
exports.validateAIControlPatch = validateAIControlPatch;
const isExecutionMode = (mode) => ['ASSISTED', 'CONTROLLED_AUTOPILOT', 'FULL_AUTOPILOT'].includes(mode);
exports.isExecutionMode = isExecutionMode;
const executionPolicyDecision = (input) => {
    const { config, definition, humanApproved = false } = input;
    if (!definition.executionAvailable)
        return { allowed: false, reason: 'ACTION_NOT_EXECUTABLE' };
    if (!(0, exports.isExecutionMode)(config.mode))
        return { allowed: false, reason: 'MODE_DOES_NOT_EXECUTE' };
    if (!config.executionEnabled)
        return { allowed: false, reason: 'EXECUTION_DISABLED' };
    if (config.emergencyPaused)
        return { allowed: false, reason: 'EMERGENCY_PAUSED' };
    if ((0, exports.normalizeConfidence)(input.confidence) < config.minimumConfidence)
        return { allowed: false, reason: 'BELOW_CONFIDENCE_THRESHOLD' };
    if (humanApproved)
        return { allowed: true, reason: config.simulationOnly ? 'HUMAN_APPROVED_SIMULATION' : 'HUMAN_APPROVED' };
    ...
    if (definition.risk === 'LOW') {
        return config.autoExecuteLowRisk
            ? { allowed: true, reason: config.simulationOnly ? 'LOW_RISK_SIMULATION' : 'LOW_RISK_POLICY' }
            : { allowed: false, reason: 'LOW_RISK_AUTO_DISABLED' };
    }
    if (definition.risk === 'MEDIUM') {
        const allowed = config.autoExecuteMediumRisk && !config.requireApprovalForMediumRisk;
        return allowed
            ? { allowed: true, reason: config.simulationOnly ? 'MEDIUM_RISK_SIMULATION' : 'MEDIUM_RISK_POLICY' }
            : { allowed: false, reason: 'MEDIUM_RISK_APPROVAL_REQUIRED' };
    }
    const allowed = config.mode === 'FULL_AUTOPILOT'
        && config.autoExecuteHighRisk
        && !config.requireApprovalForHighRisk;
    return allowed
        ? { allowed: true, reason: config.simulationOnly ? 'HIGH_RISK_SIMULATION' : 'HIGH_RISK_POLICY' }
        : { allowed: false, reason: 'HIGH_RISK_APPROVAL_REQUIRED' };
};
exports.executionPolicyDecision = executionPolicyDecision;
