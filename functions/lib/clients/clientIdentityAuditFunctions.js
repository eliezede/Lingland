...
const SAFE_RUNTIME = { timeoutSeconds: 120, memory: '512MB' };
const MERGE_RUNTIME = { timeoutSeconds: 300, memory: '1GB' };
...
const prepareMergeContext = async (candidateId, canonicalClientId, fieldSelections = {}) => {
    ...
    const documents = sourceCandidate.clientIds.map(id => {
    ...
    const hierarchyVersions = hierarchyDocuments.map(document => ({
    ...
    return {
        ...
        hierarchySnapshots,
        linkedSourceUsers,
        preview: (0, clientMergeCore_1.buildClientMergePreview)(candidate, canonicalClientId, documents, dependencies, hierarchyVersions, fieldSelections),
    };
};
const persistAuditEvent = async (data) => {
    await db.collection('clientIdentityAuditEvents').add({
        ...data,
        occurredAt: nowIso(),
    });
};
const restoreFieldPatch = (original, touchedFields) => {
    const patch = {};
...
const normalizeFieldSelections = (value) => {
 ...
 */
exports.getClientIdentityAudit = functions
...
exports.executeClientMerge = functions
    ...
    try {
        ...
        const hierarchyWrites = [];
        preview.hierarchy.departments.forEach(department => {
            const collection = 'clientDepartments';
        ...
        preview.hierarchy.memberships.forEach(membership => {
            const collection = 'clientMemberships';
        ...
        for (const dependencyChunk of chunks(merge.dependencies, 225)) {
            ...
            dependencyChunk.forEach(dependency => {
                ...
                if (bookingAgentId)
                    linkedBookingAgents += 1;
                if (bookingDepartmentId)
                    linkedBookingDepartments += 1;
            });
            await batch.commit();
...
exports.rollbackClientMerge = functions
    .runWith(MERGE_RUNTIME)
    .https.onCall(async (data, context) => {
    const actor = await assertActiveAdmin(context.auth?.uid, true);
    ...
    const confirmation = text(data?.confirmation).toUpperCase();
    if (!manifestId)
        throw new functions.https.HttpsError('invalid-argument', 'A merge manifest is required.');
    ...
    if (!manifest.exists)
        throw new functions.https.HttpsError('not-found', 'Merge manifest not found.');
    ...
    for (const dependencyChunk of chunks(dependencyBackups.docs, 400)) {
        ...
        currentDocuments.forEach((current, index) => {
            ...
            batch.update(current.ref, restoreFieldPatch(previousValues, touchedFields));
    ...
    for (const hierarchyChunk of chunks(hierarchyBackups.docs, 400)) {
        ...
        currentDocuments.forEach((current, index) => {
            ...
            batch.update(current.ref, restoreFieldPatch((backup.data || {}), Array.isArray(backup.touchedFields) ? backup.touchedFields.map(String) : []));
...
//# sourceMappingURL=clientIdentityAuditFunctions.js.map