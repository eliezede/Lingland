...
exports.reconcileXeroAccounting = void 0;
...
const cleanText = (value, max = 500) => String(value ?? '')
    ...
    .trim()
    .slice(0, max);
const upper = (value, max = 500) => cleanText(value, max).toUpperCase();
const finite = (value) => {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? Math.round(parsed * 100) / 100 : 0;
};
const normalizeName = (value) => cleanText(value, 300)
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/&/g, ' and ')
...
const strings = (value) => (Array.isArray(value) ? value.map(item => cleanText(item, 320)).filter(Boolean) : []);
const indexBy = (records, keyFor) => {
    const index = new Map();
...
const normalizeLocalContact = (record) => ({
...
const normalizeXeroContact = (record) => ({
...
const normalizeLocalDocument = (record) => {
    ...
    const externalNumber = cleanText(record.externalAccountingNumber
        || record.xeroInvoiceNumber
        || (direction === 'RECEIVABLE' ? documentNumber : (auditNumber ? `SAGE-PI-${auditNumber}` : '')), 140);
    return {
        id: cleanText(record.id || record.__id, 180),
        direction,
...
const normalizeXeroInvoice = (record, contactById) => {
...
const normalizeXeroPayment = (record) => {
...
const increment = (counts, status) => {
    counts.TOTAL += 1;
    counts[status] += 1;
};
const xeroContactSnapshot = (contact) => ({
    id: contact.id,
...
const invoiceSnapshot = (invoice, paymentsByInvoice) => {
...
const matchContacts = (localContacts, xeroContacts) => {
...
const matchDocuments = (localDocuments, xeroInvoices, contactItems, paymentsByInvoice) => {
    const byNumber = indexBy(xeroInvoices, invoice => `${invoice.type}|${upper(invoice.number, 140)}`);
    const byFingerprint = indexBy(xeroInvoices, invoice => (`${invoice.type}|${invoice.date}|${invoice.total.toFixed(2)}|${invoice.contactAccountRef}`));
    const exactContactLinks = new Map(contactItems
    ...
    localDocuments.forEach(local => {
        ...
        if (numbered.length === 1) {
            ...
            if (Math.abs(candidate.total - local.total) > 0.01)
                reasons.push('TOTAL_MISMATCH');
            if (local.date && candidate.date && candidate.date !== local.date)
                reasons.push('DATE_MISMATCH');
            const expectedXeroContactId = exactContactLinks.get(local.accountingContactId);
            const contactMatches = Boolean((expectedXeroContactId && candidate.contactId === expectedXeroContactId)
                || (local.accountRef && candidate.contactAccountRef === local.accountRef));
            if (!contactMatches)
                reasons.push('CONTACT_MISMATCH');
...
const reconcileXeroAccounting = (input) => {
    ...
    items.forEach(item => {
        if (item.entityType === 'CONTACT')
            increment(contacts, item.status);
        else {
    ...
    const hashSource = items
        ...
        .sort()
        .join('\n');
    const previewHash = (0, node_crypto_1.createHash)('sha256').update(hashSource).digest('hex');
...
exports.reconcileXeroAccounting = reconcileXeroAccounting;
//# sourceMappingURL=xeroReconciliationCore.js.map