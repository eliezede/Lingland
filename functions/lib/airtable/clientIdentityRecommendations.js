...
exports.recommendCanonicalClient = void 0;
...
const shared = (left, right) => left.find(value => right.includes(value)) || '';
const normalizeAccountKey = (value) => text(value).toUpperCase().replace(/[^A-Z0-9]/g, '');
const prepareProfile = (profile) => {
    const rawNames = unique([profile.label, ...(profile.names || [])]);
    const names = unique(rawNames
        .map(clientIdentityAuditCore_1.normalizeOrganizationName)
        .filter(value => value && !(0, clientIdentityAuditCore_1.isGenericOrganizationName)(value)));
    const emails = unique((profile.emails || []).flatMap(clientIdentityAuditCore_1.extractClientEmails));
    const addresses = unique((profile.addresses || []).map(clientIdentityAuditCore_1.normalizeClientAddress));
    return {
        id: profile.id,
        label: text(profile.label) || rawNames[0] || profile.id,
        names,
        accountKeys: unique((profile.accountKeys || []).map(normalizeAccountKey)),
        domains: (0, clientIdentityAuditCore_1.extractOrganizationDomains)(emails),
        phones: unique((profile.phones || []).map(clientIdentityAuditCore_1.normalizeClientPhone)),
        addresses,
        postcodes: unique((profile.addresses || []).map(clientIdentityAuditCore_1.extractUkPostcode)),
    };
};
const bestNameSimilarity = (left, right) => left.names.reduce((best, leftName) => Math.max(best, ...right.names.map(rightName => (0, clientIdentityAuditCore_1.organizationNameSimilarity)(leftName, rightName))), 0);
const rankTarget = (source, target) => {
    ...
    if (!exactName && nameSimilarity >= 0.68)
        evidence.push({
            code: 'SIMILAR_NAME',
    ...
    const score = Math.min(100, Math.round((sameAccountKey ? 55 : 0)
...
const recommendCanonicalClient = (sourceProfile, targetProfiles) => {
...
exports.recommendCanonicalClient = recommendCanonicalClient;
//# sourceMappingURL=clientIdentityRecommendations.js.map