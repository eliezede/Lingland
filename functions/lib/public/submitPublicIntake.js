...
const canonicalClientDocument = async (document) => {
    ...
    for (let depth = 0; depth < 4; depth += 1) {
        ...
        if (!redirectId || redirectId === current.id)
            return current;
...
const resolvePublicOrganization = async (organizationName) => {
    ...
    const matchedDocuments = Array.from(new Map([...normalizedMatches.docs, ...literalMatches.docs].map(document => [document.id, document])).values());
...
exports.submitClientBookingRequest = functions.runWith({
    ...
    const access = await (0, clientPortalAccess_1.resolveClientPortalAccess)(context.auth.uid, userData);
