...
exports.adminAssignmentAction = void 0;
...
exports.adminAssignmentAction = functions.https.onCall(async (data, context) => {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (action === 'ASSIGN') {
            ...
            assignments.docs.forEach(item => transaction.update(item.ref, String(item.data().interpreterId || '') === interpreterId
        ...
        if (action === 'UNASSIGN') {
            if (!interpreterId)
                return { status: currentStatus, idempotent: true };
        ...
        if (action === 'RECORD_RESPONSE') {
            if (!interpreterId || !interpreter?.exists)
                throw new functions.https.HttpsError('failed-precondition', 'No interpreter is assigned');
        ...
        if (action === 'COMPLETE_SESSION') {
            if (currentStatus === 'SESSION_COMPLETED')
                return { status: currentStatus, idempotent: true };
    ...
    if (emailStatus && bookingForEmail) {
        ...
        await (0, bookingEmail_1.queueBookingStatusEmails)(bookingId, bookingForEmail, emailStatus, {
...
//# sourceMappingURL=adminAssignmentActions.js.map