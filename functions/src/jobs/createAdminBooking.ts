...
const nonNegativeNumber = (value: unknown, max: number) => {
...
const allocateJobNumber = async (language: string) => db.runTransaction(async transaction => {
...
export const createAdminBooking = functions.https.onCall(async (data, context) => {
  ...
  if (interpreterId && !interpreter?.exists) throw new functions.https.HttpsError('not-found', 'Selected interpreter not found');
  ...
  const sourceFiles = (Array.isArray(data?.sourceFiles) ? data.sourceFiles : []).slice(0, 50).map((file: any) => ({
  ...
  if (assignmentRef) batch.set(assignmentRef, {
  ...
  await queueBookingStatusEmails(bookingRef.id, booking, 'INCOMING', {}, bookingRef.id);
  if (interpreterId) await queueBookingStatusEmails(bookingRef.id, booking, status, {
