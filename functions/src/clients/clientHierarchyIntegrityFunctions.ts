...
export const getClientInvoiceBookingRepairPreview = functions.runWith(RUNTIME).https.onCall(async (data, context) => {
  await assertAdmin(context.auth?.uid);
  ...
  const input = await loadIntegrityInput();
  const plan = buildClientInvoiceBookingRepairPlan(input, invoiceId, clientId);
  return {
    success: true,
    readOnly: true,
    truncated: input.truncated === true,
...
export const applyClientInvoiceBookingRepair = functions.runWith(RUNTIME).https.onCall(async (data, context) => {
  const actor = await assertAdmin(context.auth?.uid, true);
  const invoiceId = text(data?.invoiceId);
  const clientId = text(data?.clientId);
  const expectedFingerprint = text(data?.expectedFingerprint);
  ...
  if (plan.repairableBookingCount === 0 || plan.repairableBookingCount > MAX_INVOICE_REPAIR_JOBS) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      `The reviewed invoice batch must contain between 1 and ${MAX_INVOICE_REPAIR_JOBS} changed jobs.`,
    );
  }
  ...
  await db.runTransaction(async transaction => {
    ...
    currentBookings.forEach((current, index) => {
      const planned = plan.jobs[index];
      if (!current.exists) throw new functions.https.HttpsError('not-found', `Job ${planned.reference} no longer exists.`);
      const currentData = current.data() || {};
      const currentFingerprint = createBookingHierarchyFingerprint(planned.bookingId, currentData);
      ...
      if (planned.departmentName) nextSnapshot.departmentName = planned.departmentName;
      else delete nextSnapshot.departmentName;
      if (planned.requesterName) nextSnapshot.requesterName = planned.requesterName;
      else delete nextSnapshot.requesterName;
      if (planned.requesterEmail) nextSnapshot.requesterEmail = planned.requesterEmail;
      ...
      if (planned.nextClientDepartmentId) nextData.clientDepartmentId = planned.nextClientDepartmentId;
      else delete nextData.clientDepartmentId;
      if (planned.nextRequestedByAgentId) nextData.requestedByAgentId = planned.nextRequestedByAgentId;
      else delete nextData.requestedByAgentId;
      if (planned.nextRequestedByUserId) nextData.requestedByUserId = planned.nextRequestedByUserId;
      else delete nextData.requestedByUserId;
      const appliedFingerprint = createBookingHierarchyFingerprint(planned.bookingId, nextData);
...
export const rollbackClientInvoiceBookingRepair = functions.runWith(RUNTIME).https.onCall(async (data, context) => {
  const actor = await assertAdmin(context.auth?.uid, true);
  const manifestId = text(data?.manifestId);
  if (!manifestId) throw new functions.https.HttpsError('invalid-argument', 'An invoice job repair manifest is required.');
  ...
  return db.runTransaction(async transaction => {
    const manifest = await transaction.get(manifestRef);
    if (!manifest.exists) throw new functions.https.HttpsError('not-found', 'Invoice job repair manifest not found.');
    const manifestData = manifest.data() || {};
    ...
    currentBookings.forEach((current, index) => {
      const backup = backupDocuments[index].data();
      if (!current.exists) throw new functions.https.HttpsError('not-found', `Job ${text(backup.reference || current.id)} no longer exists.`);
      const currentData = current.data() || {};
      if (
        text(currentData.lastHierarchyRepairManifestId) !== manifestId
        || createBookingHierarchyFingerprint(current.id, currentData) !== text(backup.appliedFingerprint)
      ) {
