...
const enforceRequesterLookupRateLimit = async (
  context: functions.https.CallableContext,
  email: string,
) => {
  const uid = context.auth?.uid || '';
  const ref = db.collection('publicSubmissionLimits').doc(
    createHash('sha256').update(`REQUESTER_LOOKUP:${uid}`).digest('hex'),
  );
  const now = Date.now();
  await db.runTransaction(async transaction => {
    ...
    if (lastSubmittedAt && now - lastSubmittedAt < 1_000) {
...
export const lookupPublicRequesterContext = functions.runWith({
  timeoutSeconds: 60,
  memory: '256MB',
  ...
  if (agentMatches.empty) return { status: 'NO_MATCH', organizations: [] };
  if (agentMatches.size !== 1) return { status: 'AMBIGUOUS', organizations: [] };
  ...
  type CandidateMembership = {
    client: FirebaseFirestore.DocumentSnapshot;
    membershipId: string;
    membershipStatus: string;
    unrestricted: boolean;
    requestedDepartmentIds: Set<string>;
  };
  const candidates = new Map<string, CandidateMembership>();
  ...
  const organizations = (await Promise.all(Array.from(candidates.entries()).map(async ([clientId, candidate]) => {
    ...
    const allowedDepartments = activeDepartments.filter(department => (
      candidate.unrestricted || candidate.requestedDepartmentIds.has(department.id)
    ));
    return {
      id: clientId,
  ...
  if (organizations.length === 0) return { status: 'NO_MATCH', organizations: [] };
  const now = Date.now();
  const tokenRef = db.collection('publicRequesterContexts').doc();
  const tokenData: PublicRequesterContextTokenData = {
    version: 1,
    authUid: context.auth!.uid,
    emailHash: createHash('sha256').update(email).digest('hex'),
...
const resolveSubmittedRequesterContext = async (
  rawContext: unknown,
  context: functions.https.CallableContext,
  email: string,
) => {
  ...
  const tokenData = tokenDocument.data() as PublicRequesterContextTokenData;
  ...
  const validation = validatePublicRequesterSelection(tokenData, {
    authUid: context.auth?.uid || '',
    emailHash: createHash('sha256').update(email).digest('hex'),
  ...
  if (!clientDocument.exists) throw new functions.https.HttpsError('failed-precondition', 'The selected organisation is no longer available.');
...
export const submitPublicBookingRequest = functions.runWith({
  ...
  if (departmentRequestRef) {
    intakeBatch.set(departmentRequestRef, {
      ...
      normalizedName: normalizePublicDepartmentName(proposedDepartmentName),
      ...
      requesterEmailHash: createHash('sha256').update(email).digest('hex'),
      status: 'PENDING',
      sourceSystem: 'PUBLIC_INTAKE',
      organizationId: 'lingland-main',
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
...
export const adminResolveClientDepartmentRequest = functions.runWith({
  timeoutSeconds: 60,
  memory: '256MB',
}).https.onCall(async (data, context) => {
  ...
  if (!requestSnapshot.exists) throw new functions.https.HttpsError('not-found', 'Department request not found.');
  ...
  const normalizedName = normalizePublicDepartmentName(requestedName);
  ...
  if (action === 'APPROVE') {
    const departments = await db.collection('clientDepartments').where('clientId', '==', clientId).get();
    const existing = departments.docs.find(document => (
      cleanString(document.data().status || 'ACTIVE', 40).toUpperCase() === 'ACTIVE'
      && normalizePublicDepartmentName(document.data().name) === normalizedName
    ));
  ...
  await db.runTransaction(async transaction => {
    ...
    if (!booking.exists) throw new functions.https.HttpsError('not-found', 'The linked booking no longer exists.');
    ...
    if (action === 'APPROVE') {
      ...
      if (department.exists && (
        cleanString(department.data()?.status || 'ACTIVE', 40).toUpperCase() !== 'ACTIVE'
        || normalizePublicDepartmentName(department.data()?.name) !== normalizedName
      )) {
