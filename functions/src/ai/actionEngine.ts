...
import { executeActionTool, rollbackActionTool, verifyActionOutcome } from './actionTools';
import { AI_ACTION_REGISTRY, executionPolicyDecision, isKnownAIAction, mergeAIControlConfig } from './policy';
import { AIActor, AIControlConfig, AIExecutionRecord } from './types';

const nowIso = () => new Date().toISOString();
const clean = <T>(value: T): T => JSON.parse(JSON.stringify(value)) as T;
const safeError = (error: unknown) => (error instanceof Error ? error.message : 'Execution failed.')
  .replace(/[\u0000-\u001f\u007f]/g, ' ')
  .replace(/\s+/g, ' ')
  .trim()
  .slice(0, 300);

const executionIdFor = (suggestionId: string, action: string) => `exec_${createHash('sha256')
  .update(`lingland-ai-execution-v1|${suggestionId}|${action}`)
  .digest('hex')
  .slice(0, 32)}`;

const writeExecutionAudit = async (db: admin.firestore.Firestore, input: {
  actor: AIActor;
  executionId: string;
  suggestionId: string;
  mode: string;
...
const loadFreshConfig = async (db: admin.firestore.Firestore) => {
  const snapshot = await db.collection('system').doc('aiControl').get();
  return mergeAIControlConfig(snapshot.data());
};

const executionsToday = async (db: admin.firestore.Firestore) => {
...
export const executeAISuggestionAction = async (input: {
  db: admin.firestore.Firestore;
  suggestionId: string;
  actor: AIActor;
  humanApproved?: boolean;
  trigger: 'HUMAN_APPROVAL' | 'MANUAL_EXECUTION' | 'AUTO_REVIEW' | 'SCHEDULED_REVIEW';
}): Promise<ExecuteSuggestionResult> => {
  ...
  if (!initialSuggestion.exists) throw new Error('AI suggestion not found.');
  const suggestion = initialSuggestion.data() || {};
  const action = String(suggestion.action || '');
  if (!isKnownAIAction(action)) throw new Error('AI suggestion contains an unsupported action.');
  const definition = AI_ACTION_REGISTRY[action];
  const freshConfig = await loadFreshConfig(input.db);
  const decision = executionPolicyDecision({
    config: freshConfig,
    definition,
    confidence: Number(suggestion.confidence) || 0,
    humanApproved: input.humanApproved,
  });
  if (!decision.allowed) return { success: false, status: 'BLOCKED', reason: decision.reason };
  ...
  const parameters = suggestion.proposedParameters && typeof suggestion.proposedParameters === 'object'
    ? clean(suggestion.proposedParameters as Record<string, unknown>)
    : {};
  const baseExecution: AIExecutionRecord = {
    id: executionId,
    suggestionId: input.suggestionId,
  ...
  await input.db.runTransaction(async transaction => {
    ...
    if (!latestSuggestion.exists) throw new Error('AI suggestion no longer exists.');
    const latestConfig = mergeAIControlConfig(configSnapshot.data());
    const latestDecision = executionPolicyDecision({
      config: latestConfig,
      definition,
  ...
  if (idempotent) return { success: true, executionId, status: 'EXECUTING', idempotent: true };
...
export const rollbackAIExecution = async (input: {
  db: admin.firestore.Firestore;
  executionId: string;
  actor: AIActor;
}) => {
  const executionRef = input.db.collection('aiActionExecutions').doc(input.executionId);
  let execution: FirebaseFirestore.DocumentData = {};
  await input.db.runTransaction(async transaction => {
    const snapshot = await transaction.get(executionRef);
    if (!snapshot.exists) throw new Error('AI execution not found.');
    ...
    if (!isKnownAIAction(String(execution.action || ''))) {
  ...
  if (!isKnownAIAction(action)) throw new Error('Execution action is not supported.');
  const definition = AI_ACTION_REGISTRY[action];
  try {
    const summary = await rollbackActionTool({
      db: input.db,
      executionId: input.executionId,
      handler: definition.handler,
...
export const verifyAIExecutionOutcomes = async (input: {
  db: admin.firestore.Firestore;
  actor: AIActor;
  limit?: number;
}) => {
  ...
  for (const doc of snapshot.docs) {
    ...
    const action = String(execution.action || '');
    if (!isKnownAIAction(action)) continue;
    checked += 1;
    const result = await verifyActionOutcome({
      db: input.db,
      executionId: doc.id,
      handler: AI_ACTION_REGISTRY[action].handler,
      entityId: String(execution.entityId || ''),
      afterSnapshot: execution.afterSnapshot || null,
    });
    ...
    await Promise.all([
      ...
      writeExecutionAudit(input.db, {
        ...
        mode: String(execution.mode || ''),
        action,
        risk: String(execution.risk || AI_ACTION_REGISTRY[action].risk),
        entityType: String(execution.entityType || ''),
        entityId: String(execution.entityId || ''),
...
export const executeAutomaticSuggestions = async (input: {
  db: admin.firestore.Firestore;
  ...
  const results: ExecuteSuggestionResult[] = [];
  for (const suggestionId of input.suggestionIds.slice(0, input.config.maxActionsPerRun)) {
    try {
      results.push(await executeAISuggestionAction({ db: input.db, suggestionId, actor: input.actor, trigger: input.trigger }));
    } catch (error) {
