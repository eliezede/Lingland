...
export interface AIActionToolResult {
...
const entityLink = (entityType: string, entityId: string) => {
  if (entityType === 'BOOKING') return `/admin/bookings/${encodeURIComponent(entityId)}`;
...
const createInternalAlert = async (input: AIActionToolInput): Promise<AIActionToolResult> => {
  const admins = await input.db.collection('users').where('role', 'in', ['ADMIN', 'SUPER_ADMIN']).get();
  const activeAdmins = admins.docs.filter(doc => doc.data()?.status === 'ACTIVE');
  const batch = input.db.batch();
  const createdAt = nowIso();
...
const placeJobOnHold = async (input: AIActionToolInput): Promise<AIActionToolResult> => {
  if (input.entityType !== 'BOOKING') throw new Error('PLACE_JOB_ON_HOLD requires a booking.');
  ...
  await input.db.runTransaction(async transaction => {
    ...
    if (!allowed.has(status)) throw new Error(`Job cannot be placed on hold while ${status}.`);
    before = {
      status,
...
const offerInterpreter = async (input: AIActionToolInput): Promise<AIActionToolResult> => {
  if (input.entityType !== 'BOOKING') throw new Error('OFFER_INTERPRETER requires a booking.');
  const bookingRef = input.db.collection('bookings').doc(input.entityId);
  const initial = await bookingRef.get();
  if (!initial.exists) throw new Error('Booking not found.');
  ...
  const ranked = requestedId
    ? await rankInterpreterForBooking(input.db, input.entityId, initialBooking, requestedId)
    : await findBestInterpreterForBooking(input.db, input.entityId, initialBooking, offeredIds);
  const interpreterId = ranked?.id || '';
  if (!interpreterId) throw new Error('No eligible professional satisfies the assignment policy.');
  ...
  await input.db.runTransaction(async transaction => {
    ...
    if (!interpreterSnapshot.exists) throw new Error('Selected professional not found.');
    ...
    if (booking.interpreterId && String(booking.interpreterId) !== interpreterId) throw new Error('Job already has another professional.');
  ...
  if (input.externalCommunicationEnabled && bookingForEmail && interpreterForEmail) {
    ...
    try {
      await queueBookingStatusEmails(input.entityId, bookingForEmail, 'PENDING_ASSIGNMENT', {
...
const createClientInvoiceDraft = async (input: AIActionToolInput): Promise<AIActionToolResult> => {
  if (input.entityType !== 'BOOKING') throw new Error('CREATE_CLIENT_INVOICE_DRAFT requires a booking.');
  const bookingRef = input.db.collection('bookings').doc(input.entityId);
  ...
  if (candidates.length === 0) throw new Error('No approved uninvoiced timesheet is available for this job.');
  const clientId = String(booking.clientId || candidates[0].data().clientId || '');
  if (!clientId) throw new Error('The booking has no linked client.');
  ...
  await input.db.runTransaction(async transaction => {
    ...
    if (eligible.length === 0) throw new Error('Eligible timesheets were already claimed.');
    ...
    if (!Number.isFinite(vatRate) || vatRate < 0 || vatRate > 1) throw new Error('Finance VAT configuration is invalid.');
    const subtotal = Number(eligible.reduce((sum, doc) => sum + Number(doc.data()?.clientAmountCalculated || 0), 0).toFixed(2));
    if (subtotal <= 0) throw new Error('Invoice subtotal must be greater than zero.');
...
export const executeActionTool = async (input: AIActionToolInput): Promise<AIActionToolResult> => {
...
export const rollbackActionTool = async (input: {
  db: admin.firestore.Firestore;
  executionId: string;
  ...
  if (input.handler === 'CREATE_OPERATION_TASK') {
    ...
    if (!ref) throw new Error('Operation task not found for rollback.');
  ...
  if (input.handler === 'OFFER_INTERPRETER') {
    ...
    if (!assignmentId) throw new Error('Assignment reference is missing.');
    ...
    await input.db.runTransaction(async transaction => {
      ...
      if (!booking.exists || !assignment.exists) throw new Error('Booking or assignment no longer exists.');
  ...
  if (input.handler === 'CREATE_CLIENT_INVOICE_DRAFT') {
    ...
    if (!invoiceId) throw new Error('Invoice reference is missing.');
    ...
    const priorTimesheets = Array.isArray(input.beforeSnapshot?.timesheets) ? input.beforeSnapshot?.timesheets as Array<Record<string, unknown>> : [];
    ...
    batch.set(input.db.collection('bookings').doc(input.entityId), {
      status: String((input.beforeSnapshot?.booking as Record<string, unknown> | undefined)?.status || 'READY_FOR_INVOICE'),
      paymentStatus: String((input.beforeSnapshot?.booking as Record<string, unknown> | undefined)?.paymentStatus || 'READY_FOR_INVOICE'),
      clientInvoiceId: admin.firestore.FieldValue.delete(),
...
export const verifyActionOutcome = async (input: {
  db: admin.firestore.Firestore;
  executionId: string;
  handler: AIActionDefinition['handler'];
  entityId: string;
