...
import { AIReviewScope, AISuggestionDraft } from './types';
import { opaqueEntityId } from './policy';

type EntityType = AISuggestionDraft['entityType'];
...
export interface AIJobRecord {
  ...
  hasInterpreterInvoice: boolean;
  hasCostCode: boolean;
  clientAmount: number | null;
  professionalCost: number | null;
}
...
export interface AIInvoiceRecord {
  ...
  opaqueId: string;
  entityLabel: string;
  entityType: 'CLIENT_INVOICE' | 'INTERPRETER_INVOICE';
  status: string;
...
export interface AISyncConflictRecord {
  localId: string;
  opaqueId: string;
  entityLabel: string;
  severity: string;
...
export interface AIReviewContext {
  ...
  generatedAt: string;
  jobs: AIJobRecord[];
  clientInvoices: AIInvoiceRecord[];
  interpreterInvoices: AIInvoiceRecord[];
  syncConflicts: AISyncConflictRecord[];
  entityLookup: Record<string, { entityType: EntityType; entityId: string; entityLabel: string }>;
  providerContext: Record<string, unknown>;
  dataSummary: Record<string, number>;
}

const token = (value: unknown, fallback = '', max = 60) => {
...
const normalizedLabel = (data: Record<string, unknown>, id: string) => token(
  data.displayRef || data.jobNumber || data.bookingRef || data.reference,
  `Record ${id.slice(0, 8)}`,
  80,
);

const shouldReadJobs = (scope: AIReviewScope) => ['JOBS', 'ALLOCATION', 'BILLING', 'COST', 'PLATFORM'].includes(scope);
const shouldReadInvoices = (scope: AIReviewScope) => ['BILLING', 'COST', 'PLATFORM'].includes(scope);
const shouldReadSync = (scope: AIReviewScope) => ['SYNC', 'PLATFORM'].includes(scope);

const emptySnapshot = () => ({ docs: [] as admin.firestore.QueryDocumentSnapshot[] });

export const buildAIReviewContext = async (scope: AIReviewScope): Promise<AIReviewContext> => {
  ...
  const entityLookup: AIReviewContext['entityLookup'] = {};

  const jobs = jobsSnapshot.docs.map(doc => {
    const data = doc.data() as Record<string, unknown>;
    const opaqueId = opaqueEntityId('BOOKING', doc.id);
  ...
  const mapInvoices = (
    ...
    entityType: AIInvoiceRecord['entityType'],
  ): AIInvoiceRecord[] => docs.map(doc => {
    const data = doc.data() as Record<string, unknown>;
    const opaqueId = opaqueEntityId(entityType, doc.id);
    const entityLabel = normalizedLabel(data, doc.id);
  ...
  const syncConflicts = conflictSnapshot.docs.map(doc => {
    const data = doc.data() as Record<string, unknown>;
    const opaqueId = opaqueEntityId('SYNC_CONFLICT', doc.id);
    ...
    } satisfies AISyncConflictRecord;
  });
...
const parseJobDate = (date: string, time: string): number | null => {
  if (!date) return null;
...
const scopeAllows = (scope: AIReviewScope, categories: AIReviewScope[]) => scope === 'PLATFORM' || categories.includes(scope);

export const analyseOperationalContext = (
  ...
  for (const job of context.jobs) {
    ...
    if (
      scopeAllows(context.scope, ['JOBS', 'ALLOCATION'])
      && !job.assigned
    ...
    if (
      scopeAllows(context.scope, ['BILLING', 'JOBS'])
    ...
    if (
      scopeAllows(context.scope, ['COST', 'BILLING'])
