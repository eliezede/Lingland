...
const RUNTIME = { timeoutSeconds: 540, memory: '1GB' as const };
...
const assertSuperAdmin = async (uid?: string): Promise<ActiveAdmin> => {
  if (!uid) throw new functions.https.HttpsError('unauthenticated', 'Authentication is required.');
  const snapshot = await db.collection('users').doc(uid).get();
  const data = snapshot.data() || {};
  if (!snapshot.exists || data.status !== 'ACTIVE' || text(data.role, 40).toUpperCase() !== 'SUPER_ADMIN') {
  ...
  return { uid, role: 'SUPER_ADMIN', organizationId: text(data.organizationId, 160) || DEFAULT_ORGANIZATION_ID };
};
...
const assertCommunicationsSuppressed = async () => {
  ...
  if (mode !== 'SUPPRESSED') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Sage migration writes require Communication Mode SUPPRESSED.',
    );
  }
};

const runRef = (runId: string) => db.collection('accountingImportRuns').doc(runId);
const batchRef = (runId: string, module: SageImportModule, batchIndex: number) => (
  runRef(runId).collection('batches').doc(`${module}_${String(batchIndex).padStart(5, '0')}`)
);

const requireRun = async (runId: string) => {
  const snapshot = await runRef(runId).get();
  if (!snapshot.exists) throw new functions.https.HttpsError('not-found', 'Sage import run not found.');
  return { snapshot, data: snapshot.data() as SageRunData };
};

const batchPayloadBytes = (records: NormalizedSageRecord[]) => Buffer.byteLength(JSON.stringify(records), 'utf8');

const writeImportEvent = async (
  runId: string,
  actor: ActiveAdmin,
  action: string,
...
export const createSageImportRun = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
  const actor = await assertSuperAdmin(context.auth?.uid);
  await assertCommunicationsSuppressed();
  let manifest: SageImportManifest;
  try {
    manifest = validateSageImportManifest(input?.manifest);
  } catch (error) {
  ...
  if (existing.exists) {
    const current = existing.data() as SageRunData;
...
export const stageSageImportBatch = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
  const actor = await assertSuperAdmin(context.auth?.uid);
  ...
  if (!runId || !isSageImportModule(module) || !Number.isInteger(batchIndex) || batchIndex < 0
  ...
  let records: NormalizedSageRecord[];
  try {
    records = rawRecords.map((record: unknown) => normalizeSageImportRecord(module, record, run.manifest.datasetId));
  } catch (error) {
  ...
  const existing = await ref.get();
  const batchHash = hashSageBatch(records);
  if (existing.exists) {
...
export const finalizeSageImportPreview = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
  const actor = await assertSuperAdmin(context.auth?.uid);
  const runId = text(input?.runId, 80);
  const { data: run } = await requireRun(runId);
  ...
  if (run.status !== 'STAGING') throw new functions.https.HttpsError('failed-precondition', 'This Sage run cannot be previewed.');

  const batches = await runRef(runId).collection('batches').get();
  const moduleCounts: Partial<Record<SageImportModule, number>> = {};
  const moduleIndexes = new Map<SageImportModule, Set<number>>();
  const moduleBatchCounts = new Map<SageImportModule, number>();
  const recordIds = new Set<string>();
  const hashes: string[] = [];
  for (const document of batches.docs) {
    const data = document.data();
    const module = data.module;
    if (!isSageImportModule(module)) throw new functions.https.HttpsError('data-loss', `Unknown staged module in ${document.id}.`);
    const batchIndex = Number(data.batchIndex);
    const batchCount = Number(data.batchCount);
    const records = Array.isArray(data.records) ? data.records as NormalizedSageRecord[] : [];
    moduleCounts[module] = (moduleCounts[module] || 0) + records.length;
    if (!moduleIndexes.has(module)) moduleIndexes.set(module, new Set());
    ...
    hashes.push(text(data.batchHash, 64));
    for (const record of records) {
      const scopedId = `${module}:${record.id}`;
      if (recordIds.has(scopedId)) throw new functions.https.HttpsError('data-loss', `Duplicate staged id ${scopedId}.`);
      recordIds.add(scopedId);
    }
  }

  for (const [module, expected] of Object.entries(run.manifest.expectedModuleCounts)) {
    const typedModule = module as SageImportModule;
  ...
  const previewHash = stableSageId('preview', [run.manifest.manifestHash, ...hashes.sort()].join('|')).replace('preview_', '');
  const preview = {
    ready: true,
...
const collectionForModule = (module: SageImportModule): string => {
  if (module === 'contacts') return 'accountingContacts';
  if (module === 'accounts') return 'accountingAccounts';
  if (module === 'bankAccounts') return 'accountingBankAccounts';
  if (module === 'salesDocuments' || module === 'purchaseDocuments') return 'accountingDocuments';
  if (module === 'customerPayments' || module === 'supplierPayments') return 'accountingPayments';
  if (module === 'bankJournalEntries') return 'accountingLedgerEntries';
  return 'accountingSourceArtifacts';
};

const canonicalClientCandidates = async (contact: NormalizedSageRecord) => {
  const sageAccountRef = text(contact.sageAccountRef, 80);
  const nameKey = text(contact.normalizedName, 200) || normalizedName(contact.name);
  const emails = Array.isArray(contact.emails) ? contact.emails.map(email => text(email, 320).toLowerCase()).filter(Boolean) : [];
  const bySage = await db.collection('clients').where('sageAccountRef', '==', sageAccountRef).limit(3).get();
  if (!bySage.empty) return bySage.docs;
  if (nameKey) {
    ...
    const active = byName.docs.filter(document => {
      const data = document.data();
      return text(data.recordState, 40).toUpperCase() !== 'MERGED' && !text(data.mergedIntoClientId, 160);
    });
    if (active.length) return active;
  }
  for (const email of emails.slice(0, 3)) {
    const byInvoiceEmail = await db.collection('clients').where('invoiceEmail', '==', email).limit(3).get();
    if (!byInvoiceEmail.empty) return byInvoiceEmail.docs;
  }
  return [];
};

const upsertCanonicalClient = async (
  contact: NormalizedSageRecord,
  runId: string,
  actor: ActiveAdmin,
) => {
  const candidates = await canonicalClientCandidates(contact);
  if (candidates.length > 1) return { status: 'NEEDS_REVIEW', clientId: '', reason: 'MULTIPLE_CANONICAL_CLIENT_MATCHES' };

  const sageAccountRef = text(contact.sageAccountRef, 80);
  const companyName = text(contact.name, 200);
  const nameKey = text(contact.normalizedName, 200) || normalizedName(companyName);
  const emails = Array.isArray(contact.emails) ? contact.emails.map(email => text(email, 320).toLowerCase()).filter(Boolean) : [];
  const phones = Array.isArray(contact.phones) ? contact.phones.map(phone => text(phone, 80)).filter(Boolean) : [];
  const addressLines = Array.isArray(contact.addressLines) ? contact.addressLines.map(line => text(line, 300)).filter(Boolean) : [];
  const clientId = candidates[0]?.id || stableSageId('sage_client', sageAccountRef);
  const ref = db.collection('clients').doc(clientId);
  const snapshot = candidates[0] || await ref.get();
  const current = snapshot.exists ? snapshot.data() || {} : {};
  const now = nowIso();
  const aliases = Array.from(new Set([
    ...(Array.isArray(current.accountAliases) ? current.accountAliases.map((alias: unknown) => text(alias, 200)) : []),
    companyName,
    sageAccountRef,
  ].filter(Boolean)));
  const payload: Record<string, unknown> = {
    id: clientId,
    organizationId: text(current.organizationId, 160) || actor.organizationId,
    companyName: text(current.companyName || current.name, 200) || companyName,
    normalizedCompanyName: text(current.normalizedCompanyName, 200) || nameKey,
    billingAddress: text(current.billingAddress, 2000) || addressLines.join(', '),
    paymentTermsDays: Number(current.paymentTermsDays) || Number(contact.paymentTermsDays) || 30,
    contactPerson: text(current.contactPerson, 160) || text(contact.contactName, 160),
    email: text(current.email, 320).toLowerCase() || emails[0] || '',
    phone: text(current.phone, 80) || phones[0] || '',
    invoiceEmail: text(current.invoiceEmail, 320).toLowerCase() || emails[0] || '',
    status: text(current.status, 40) || 'ACTIVE',
    recordState: text(current.recordState, 40) || 'ACTIVE',
    defaultCostCodeType: text(current.defaultCostCodeType, 40) || 'PO',
    sourceKey: text(current.sourceKey, 160) || sageAccountRef,
    sageAccountRef,
    accountingContactId: contact.id,
    accountAliases: aliases,
    crmCohort: text(current.crmCohort, 40) || 'CURRENT',
    crmReviewStatus: text(current.crmReviewStatus, 40) || 'CANONICAL',
    sourceSystem: text(current.sourceSystem, 60) || 'SAGE',
    sourceTable: text(current.sourceTable, 120) || text(contact.sourceTable, 120),
    sourceRecordId: text(current.sourceRecordId, 200) || text(contact.sourceRecordId, 200),
    syncStatus: text(current.syncStatus, 40) || 'SYNCED',
...
const linkSupplierProfessional = async (contact: NormalizedSageRecord, runId: string, actor: ActiveAdmin) => {
  const emails = Array.isArray(contact.emails) ? contact.emails.map(email => text(email, 320).toLowerCase()).filter(Boolean) : [];
  for (const email of emails.slice(0, 4)) {
    ...
    if (matches.size === 1) {
      await matches.docs[0].ref.set({
        sageSupplierRef: text(contact.sageAccountRef, 80),
        accountingContactId: contact.id,
        lastSageImportRunId: runId,
    ...
    if (matches.size > 1) return { status: 'NEEDS_REVIEW', interpreterId: '', reason: 'MULTIPLE_EMAIL_MATCHES' };
  }
  return { status: 'UNRESOLVED', interpreterId: '', reason: 'NO_EXACT_EMAIL_MATCH' };
};

const commitContact = async (
  record: NormalizedSageRecord,
  runId: string,
  actor: ActiveAdmin,
) => {
  const contactType = text(record.contactType, 20).toUpperCase();
  const requiresIdentityReview = text(record.identityStatus, 40).toUpperCase() === 'REVIEW_REQUIRED';
  const reviewReason = text(record.identityReviewReason, 120) || 'SOURCE_IDENTITY_REQUIRES_REVIEW';
...
export const commitSageImportBatch = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
  const actor = await assertSuperAdmin(context.auth?.uid);
  await assertCommunicationsSuppressed();
  const runId = text(input?.runId, 80);
  const module = input?.module;
  ...
  if (!snapshot.exists) throw new functions.https.HttpsError('not-found', 'Staged Sage batch not found.');
  ...
  const records = Array.isArray(batch.records) ? batch.records as NormalizedSageRecord[] : [];
  await runRef(runId).set({ status: 'COMMITTING', updatedAt: nowIso(), updatedBy: actor.uid }, { merge: true });

  const linkSummary: Record<string, number> = {};
  if (module === 'contacts') {
    for (const record of records) {
...
export const finalizeSageImportCommit = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
  const actor = await assertSuperAdmin(context.auth?.uid);
  const runId = text(input?.runId, 80);
  const { data: run } = await requireRun(runId);
  if (run.status === 'COMMITTED') return { runId, status: 'COMMITTED', summary: run.preview, idempotent: true };
  if (!run.preview?.ready) throw new functions.https.HttpsError('failed-precondition', 'The Sage preview is not ready.');
  const batches = await runRef(runId).collection('batches').get();
...
export const getSageImportRun = functions.runWith(RUNTIME).https.onCall(async (input, context) => {
