...
export const updateClientInvoiceStatus = functions.https.onCall(async (data, context) => {
  ...
  let invoiceForEmail: FirebaseFirestore.DocumentData | null = null;
  await db.runTransaction(async transaction => {
    const invoice = await transaction.get(invoiceRef);
    if (!invoice.exists) throw new functions.https.HttpsError('not-found', 'Invoice not found');
    ...
    if (nextStatus !== 'CANCELLED') {
      ...
      if (integrityIssues.length) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          `Invoice cannot progress: ${integrityIssues.join('; ')}`
        );
      }
    }
    ...
    if (nextStatus === 'SENT') invoicePatch.sentAt = now;
    if (nextStatus === 'PAID') invoicePatch.paidAt = now;
    if (nextStatus === 'CANCELLED') invoicePatch.cancelledAt = now;
  ...
  if (nextStatus === 'SENT' && invoiceForEmail) {
    const invoice: any = invoiceForEmail;
