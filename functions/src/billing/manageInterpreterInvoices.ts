...
type InvoiceActor = { uid: string; isAdmin: boolean; interpreterId: string };
...
const getInvoiceActor = async (uid?: string): Promise<InvoiceActor> => {
  if (!uid) throw new functions.https.HttpsError('unauthenticated', 'Authentication is required');
  const user = await db.collection('users').doc(uid).get();
...
const cleanReference = (value: unknown) => String(value || '').trim().slice(0, 120);
const deterministicInvoiceId = (interpreterId: string, reference: string) =>
  `upload_${createHash('sha256').update(`${interpreterId}|${reference.toLowerCase()}`).digest('hex').slice(0, 32)}`;

export const createInterpreterInvoiceUpload = functions.https.onCall(async (data, context) => {
  ...
  const timesheetIds: string[] = Array.from(new Set<string>(
    (Array.isArray(data?.timesheetIds) ? data.timesheetIds : []).map((value: unknown) => String(value)).filter(Boolean)
  ));
  ...
  const result = await db.runTransaction(async transaction => {
    ...
    if (!interpreter.exists) throw new functions.https.HttpsError('not-found', 'Interpreter not found');
    ...
    const timesheets = [] as FirebaseFirestore.DocumentSnapshot[];
    for (const ref of timesheetRefs) timesheets.push(await transaction.get(ref));
    ...
    const bookings = [] as FirebaseFirestore.DocumentSnapshot[];
    for (const ref of bookingRefs) bookings.push(await transaction.get(ref));

    const payableTotal = Number(timesheets.reduce((total, item) => {
      if (!item.exists) throw new functions.https.HttpsError('not-found', `Timesheet ${item.id} not found`);
...
export const updateInterpreterInvoiceStatus = functions.https.onCall(async (data, context) => {
  ...
  const result = await db.runTransaction(async transaction => {
    ...
    if (!invoice.exists) throw new functions.https.HttpsError('not-found', 'Interpreter invoice not found');
    ...
    if (['APPROVED', 'PAID'].includes(nextStatus)) {
      ...
      if (integrityIssues.length) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          `Interpreter invoice cannot progress: ${integrityIssues.join('; ')}`
        );
      }
    }
    ...
    const statusUpdate: Record<string, unknown> = { status: nextStatus, updatedAt: admin.firestore.FieldValue.serverTimestamp() };
