...
import { AlertTriangle, ArrowUpRight, CheckCircle, ChevronLeft, Download, FileText, PoundSterling, XCircle } from 'lucide-react';
...
export const AdminInterpreterInvoiceDetailsPage = () => {
  ...
  const rawReference = invoice.externalInvoiceReference || invoice.id;
  const referenceMissing = invoice.referenceIntegrityStatus === 'MISSING'
    || invoice.externalInvoiceReference === 'Reference missing'
  ...
  const integrityIssues = [
    Math.abs(Number(total || 0)) < 0.005 || invoice.financialIntegrityStatus === 'AMOUNT_MISSING'
      ? 'The payable amount is missing or zero.'
      : '',
    lines.length === 0 ? 'No persisted payable lines were found.' : '',
    invoice.financialIntegrityStatus === 'LINK_MISSING' || (invoice.sourceSystem === 'AIRTABLE' && summary.jobs === 0)
      ? 'The Airtable document is not linked to mirrored work.'
      : '',
    referenceMissing
      ? 'The supplier invoice reference is missing.'
      : '',
  ].filter(Boolean);
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {[InvoiceStatus.DRAFT, InvoiceStatus.SUBMITTED].includes(invoice.status) && (
            <>
              <button
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs font-semibold text-rose-800 dark:text-rose-200">
                Approval and payment are blocked until the payable has a valid amount, reference and linked work.
              </p>
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-2 border-b border-slate-200 px-4 py-3 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-sm font-black text-slate-950 dark:text-white">
            {Math.abs(Number(summary.total || total || 0)) < 0.005 ? 'Amount missing' : money(summary.total || total, currency)}
          </p>
        </div>
