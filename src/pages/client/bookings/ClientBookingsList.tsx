...
export const ClientBookingsList = () => {
  ...
  const filteredBookings = (bookings || []).filter(b => {
    const q = safe(filter);
    return safe(b.languageTo).includes(q) ||
                  ...
                  <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900">{booking.date || 'TBD'}</span>
                        <div className="flex items-center text-xs text-gray-500 mt-0.5">
                          <Clock size={12} className="mr-1" />
                          {booking.startTime || '--:--'} ({booking.durationMinutes || 0}m)
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                        {booking.languageFrom || 'Eng'} &rarr; {booking.languageTo || 'TBD'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
